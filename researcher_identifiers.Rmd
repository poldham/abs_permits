...
```{r eval=FALSE}
library(rorcid)
oldham <- orcid_works("0000-0002-1013-4390")
```
...
View the first record and note that there are a lot of columns of data that you probably won't be needing.

```{r}
oldham$`0000-0002-1013-4390`$group$`work-summary`[[1]] %>% knitr::kable()
```
...
View all works in a data frame (gets long fast as a large number of columns)

```{r eval=FALSE}
library(tidyverse)
oldham[["0000-0002-1013-4390"]][["group"]][["work-summary"]] %>% bind_rows()
```

Or use purrr to access and bind the list of data frames containing the publications and use select to just view the titles. You will normally want to add the orcid_id column. It is added here using `dplyr::add_column`

```{r}
library(tidyverse)
oldham %>% 
  modify(c("group", "work-summary")) %>%
  flatten() %>%
  bind_rows() %>% 
  select(`title.title.value`) %>% 
  add_column(., orcid_id = names(oldham)) %>% 
  knitr::kable()
```
